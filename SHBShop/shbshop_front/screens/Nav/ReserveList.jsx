...
const ReserveList = ({ route, navigation }) => {
  ...
  const data = storedata.data.receipt_list;

  const goToReserveDetail = async (sid,ownerType,rid) => {
    const Data = await AsyncStorage.getItem('UserData');
    const userData = JSON.parse(Data);
    const userId = userData.decoded_user_id;
    const Token = await AsyncStorage.getItem('jwtToken');
    const response = await fetch(`${API_URL}/shop/${userId}/${sid}/check-pr/${ownerType}/${rid}`, {
      method: 'GET',
      headers: {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.item} onPress={() => goToReserveDetail(item.sid,item.ownerType,item.rid)}>
      <Image
    ...
    <SafeAreaView style={styles.container}>
      <View style={{flexDirection:'row', alignItems:'center'}}>
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Ionicons
          name="chevron-back-outline" size={24} style={{paddingLeft:10}}></Ionicons>
          </TouchableOpacity>
